...
def _outline_text(ax, x, y, txt, color="black", **kwargs):
    """Text with thin white outline; accepts any Text kwargs."""
    t = ax.text(x, y, txt, color=color, **kwargs)
    t.set_path_effects([pe.withStroke(linewidth=2.4, foreground="white", alpha=0.95)])
    return t

def _hash_angle(name: str) -> float:
    """Deterministic angle (0..2π) from a name for stable label directions."""
...
def make_stacey_figure(
    ...
    """
    Stacey Matrix with distant labels + leader lines and zones:
      - Yellow:  0–3 × 0–3
    ...
    Labels:
      - For single points: label at fixed distance in a deterministic direction + leader line
      - For duplicate (x,y): bubbles around a ring; labels farther out along same angles + leader lines
    """
    ...
    _rect(ax, 3, 6, 0, 6, COL_BLUE)
    _rect(ax, 0, 6, 3, 6, COL_BLUE)
    _rect(ax, 6, 9, 0, 9, COL_GREEN)
    ...
    # Distances (axis units)
    RING_MIN, RING_MAX = 0.14, 0.26
    LABEL_SINGLE = 0.60
    LABEL_GROUP_EXTRA = 0.45

    for (x0, y0), g in groups:
        k = len(g)
        ring_r = RING_MIN if k == 1 else min(RING_MAX, RING_MIN + 0.05 * (k - 1))
        ...
        if k == 1:
            ...
            lx = x0 + LABEL_SINGLE * math.cos(ang)
            ly = y0 + LABEL_SINGLE * math.sin(ang)
            lx, ly = _keep_inside(lx, ly)

            ax.plot([x0, lx], [y0, ly], linewidth=0.7, alpha=0.65, color="black")
            _outline_text(ax, lx, ly, row["lab"], fontsize=8)
            continue

        # duplicates: bubbles on ring; labels farther out
        angles = [2 * math.pi * i / k for i in range(k)]
        for ang, (_, row) in zip(angles, g.iterrows()):
            bx = x0 + ring_r * math.cos(ang)
            by = y0 + ring_r * math.sin(ang)
            bx, by = _keep_inside(bx, by)

            size = max(50, math.sqrt(row["m"]) * size_scale)
            ax.scatter([bx], [by], s=[size], alpha=0.65, edgecolors="black", linewidths=0.5)

            lx = x0 + (ring_r + LABEL_GROUP_EXTRA) * math.cos(ang)
            ly = y0 + (ring_r + LABEL_GROUP_EXTRA) * math.sin(ang)
            lx, ly = _keep_inside(lx, ly)
